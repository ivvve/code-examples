...
class DistributedLockConfiguration(
    ...
    @Around("@annotation(com.tistory.devs0n.lock.config.lock.DistributedLock)")
    fun distributedLockAdvice(joinPoint: ProceedingJoinPoint): Any? {
        ...
        // get lock key from lock parameter


        val parameterAnnotationsList = methodSignature.method.parameterAnnotations
        val parametersTypes = methodSignature.method.genericParameterTypes

        for (i in parametersTypes.indices) {
            val parameterAnnotations = parameterAnnotationsList[i]
            val parameterType = parametersTypes[i] as ParameterizedType
        ...
        LOGGER.debug("Succeeded to get Lock - ${lock.name}")

        try {
            return joinPoint.proceed()
        } finally {
            lock.unlock()
            LOGGER.debug("Unlock the Lock - ${lock.name}")
        }
    }
